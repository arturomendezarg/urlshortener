...
**Postman testing:** [`docs/url-shortener-enterprise.postman_collection.json`](./docs/url-shortener-enterprise.postman_collection.json) and its companion [`docs/url-shortener-enterprise.postman_environment.json`](./docs/url-shortener-enterprise.postman_environment.json) exercise V1, V2, expiration, custom alias/reserved-slug/validation errors, device-classified redirects, and the async bulk submit+poll flow, structured as two independent folders: *Greenfield* runs V2 directly, treating it as the only system in existence (no V1 framing), matching this project's own stance that a true greenfield build would only ever have V2; *Brownfield* runs V1 first as the pre-existing baseline, then represents a 'V2 cutover' commit and asserts the Gateway resolves the new V2 code correctly on the very first try, with no prior direct read against V2 required — the fix for the cache-as-index defect this folder used to reproduce (see section 13 and `AI_USAGE_LOG.md`). `README.md` ('For reviewers') also gives the equivalent `curl` steps for a terminal-only pass, and `infra/k8s/README.md` ('Smoke test') has the transcript actually run against the Kubernetes deployment.
...
- **Fixed: the Gateway used to decide V1-vs-V2 from a cache, not from an index.** `DynamicShortCodeRoutingFilter` used to treat the presence of `shortlink:v2:<code>` in Redis as proof that a code lives in V2, but that key was `ShortLinkCache`'s read-through cache: written only when a link is *read*, with a 300-second TTL, and never written at creation. A freshly created V2 link was routed to V1 and 404'd until something read it directly, and a working V2 link started 404ing again once its entry expired — demonstrated on the running cluster, the identical request answering `404` then `302` either side of a direct read. Closed by removing the shared state instead of patching around it: the filter now asks V2 directly whether a code exists (a `HEAD /{shortCode}` probe, routing to V2 on anything other than a `404`) and falls back to V1 only when V2 says the code does not exist or cannot be reached — no Redis dependency left in the Gateway at all. Trade-off: an extra network round trip to V2 for every request the Gateway still routes to V1, a cost that shrinks on its own as more of V1's codes get cut over to V2. See `DynamicShortCodeRoutingFilter`'s own Javadoc and `infra/k8s/README.md` ("Gateway routing: V1-vs-V2") for the full detail, and `AI_USAGE_LOG.md` for why the original design chose a cache in the first place.
