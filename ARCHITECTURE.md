...
**Postman testing:** [`docs/url-shortener-enterprise.postman_collection.json`](./docs/url-shortener-enterprise.postman_collection.json) and its companion [`docs/url-shortener-enterprise.postman_environment.json`](./docs/url-shortener-enterprise.postman_environment.json) exercise V1, V2, expiration, custom alias/reserved-slug/validation errors, device-classified redirects, and the async bulk submit+poll flow, structured as two independent folders: *Greenfield* runs V2 directly, treating it as the only system in existence (no V1 framing), matching this project's own stance that a true greenfield build would only ever have V2; *Brownfield* runs V1 first as the pre-existing baseline, then represents a 'V2 cutover' commit and reproduces — live, with a soft (non-failing) assertion — the Gateway cache-as-index defect described in `infra/k8s/README.md`'s Known defect section, so the collection stays valid both before and after that fix lands. `README.md` ('For reviewers') also gives the equivalent `curl` steps for a terminal-only pass, and `infra/k8s/README.md` ('Smoke test') has the transcript actually run against the Kubernetes deployment.
